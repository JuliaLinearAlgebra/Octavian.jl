...
randdual(x, ::Val{N} = Val(3)) where {N} =
  ForwardDiff.Dual(x, ntuple(_ -> randn(), Val(N))...)
@time @testset "ForwardDiff.jl" begin
  ...
  @testset "real array from the left" begin
    ...
    J1 =
      ForwardDiff.jacobian((C, B) -> Octavian.matmul!(C, A1, B), C1, B1, config)
    @test J1 ≈ kron(I, A1)

    J2 = ForwardDiff.jacobian(
      (C, B) -> LinearAlgebra.mul!(C, A2, B),
      C2,
      B2,
      config
    )
    ...
    J3 = ForwardDiff.jacobian(
      (C, B) -> Octavian.matmul_serial!(C, A1, B),
      C1,
      B1,
      config
    )
  ...
  @testset "real array from the right" begin
    ...
    J1 =
      ForwardDiff.jacobian((C, A) -> Octavian.matmul!(C, A, B1), C1, A1, config)
    J2 = ForwardDiff.jacobian(
      (C, A) -> LinearAlgebra.mul!(C, A, B2),
      C2,
      A2,
      config
    )
    J3 = ForwardDiff.jacobian(
      (C, A) -> Octavian.matmul_serial!(C, A, B1),
      C1,
      A1,
      config
    )
